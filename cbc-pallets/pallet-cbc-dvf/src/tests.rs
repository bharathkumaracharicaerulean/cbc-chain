...
mod tests {
    use crate::mock::*;
    use crate::{
        ValidatorStates, ValidatorLeaveRequests, EpochVotingWeight, TotalVotingWeight,
        VoteRecords, DvfJustification, Error, DvfVote,
    };
    use codec::Encode;
    ...
    mod genesis_tests {
        ...
        fn genesis_voting_weights_and_round_zero_initialized() {
            new_test_ext().execute_with(|| {
                let total_weight = TotalVotingWeight::<Test>::get();
    ...
    mod weight_calculation_and_epoch_tests {
        ...
        fn freeze_epoch_weights_recalculates_voting_power() {
            new_test_ext().execute_with(|| {
                ...
                let total = TotalVotingWeight::<Test>::get();
                assert!(total > 0);
            });
        }
